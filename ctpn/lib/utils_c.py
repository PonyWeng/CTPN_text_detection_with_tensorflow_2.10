...
def bbox_transfor_inv(anchor, regr):
    ...
    Cyx = Vcx * ha + Cya  # GTBox y
    hx = np.exp(Vhx) * ha  # GTBox 
...
def get_session(gpu_fraction=0.6):
    ...
    if gpus:
        try:
            # TensorFlow 2.x 不再使用 tf.Session 和 tf.ConfigProto。
            # 相反地，您直接設置 GPU 配置。
            
            # 設置 GPU 記憶體增長選項，避免一開始就佔用所有記憶體
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)
            
            # 計算所需分配的記憶體量並設置
            total_memory = gpus[0].memory_info().total
            memory_limit = total_memory * gpu_fraction
            tf.config.experimental.set_virtual_device_configuration(
                gpus[0],
                [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=memory_limit)]
            )
            
            # TensorFlow 2.x 中，您不需要手動創建 session。
            # 直接使用 tf.function (自動圖模式) 或者 Eager Execution (即時執行模式)。
            
            # 如果需要在特定操作中使用多線程，可以透過以下方式設置
            num_threads = os.environ.get('OMP_NUM_THREADS')
            if num_threads:
