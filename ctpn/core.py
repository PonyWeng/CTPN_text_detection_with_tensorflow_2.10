...
def _rpn_loss_regr(y_true, y_pred):
    ...
    cls = y_true[0, :, 0]
    regr = y_true[0, :, 1:3]
    regr_keep = tf.compat.v1.where(K.equal(cls, 1))[:, 0]
    ...
    return K.switch(tf.size(input=loss) > 0, K.mean(loss), K.constant(0.0))
...
def _rpn_loss_cls(y_true, y_pred):
    ...
    """
    y_true = y_true[0][0]
    cls_keep = tf.compat.v1.where(tf.not_equal(y_true, -1))[:, 0]
    ...
    return K.switch(tf.size(input=loss) > 0, K.clip(K.mean(loss), 0, 10), K.constant(0.0))
